...
function Index() {
  ...
  useEffect(() => {
    if (!active) return;
    const fresh = items.find((item) => item.id === active.id);
    if (fresh) setActive(fresh as Run);
  }, [items, active?.id]);
  ...
  const downloadAll = () => {
    ...
    a.href = url;
    a.download = `${active.title.toLowerCase().replace(/[^a-z0-9áéíóúüñ]+/gi, "-").replace(/^-|-$/g, "") || "repurpose"}.txt`;
    a.click();
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,0.86fr)_minmax(0,1.14fr)] lg:gap-10">
        ...
        <div className="rise space-y-4" style={{ animationDelay: "160ms" }}>
          {active ? (
            <>
              <div className="hairline flex flex-wrap items-center justify-between gap-3 rounded-md bg-ink-raise/40 px-4 py-3">
                ...
                <div className="flex shrink-0 flex-wrap items-center gap-2">
                  <span className="label-mono flex items-center gap-1.5 px-2"><Layers className="size-3.5" /> {active.formats.length}</span>
                  <span className="label-mono flex items-center gap-1.5 px-2"><Clock className="size-3.5" /> {(active.durationMs / 1000).toFixed(1)}s</span>
                  <span className="label-mono flex items-center gap-1.5 px-2"><Cpu className="size-3.5" /> {active.model.split("/")[1] ?? active.model}</span>
                  <button onClick={downloadAll} title="Descargar todo" className="flex items-center gap-1.5 rounded-sm border border-ink-line px-2 py-1.5 font-mono text-[10px] uppercase tracking-widest text-paper-dim transition hover:border-acid/40 hover:text-acid">
                    <Download className="size-3.5" /> todo
                  </button>
                </div>
              </div>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              {formats.map((k, i) => <div key={k} className="hairline rise h-52 animate-pulse rounded-md bg-ink-raise/40" style={{ animationDelay: `${i * 70}ms` }} />)}
            </div>
          ) : (
            <div className="hairline flex min-h-[26rem] flex-col justify-center rounded-md bg-ink-raise/25 p-8">
              ...
              <h2 className="font-display mb-3 text-3xl">Aquí aparecerán tus <span className="text-acid italic">piezas</span></h2>
              <p className="mb-6 max-w-md text-[14.5px] text-paper-dim">Cada formato se genera con su propio brief editorial: estructura, longitud y ritmo distintos. Todos se ejecutan en paralelo, así que seis piezas tardan lo mismo que una.</p>
              <ul className="space-y-2">
                {allFormats.map((f) => <li key={f.key} className="flex items-baseline gap-3 border-t border-ink-line pt-2"><span className="text-[14.5px] text-paper/85">{f.label}</span><span className="label-mono">{f.hint}</span></li>)}
              </ul>
            </div>
          )}
        </div>
