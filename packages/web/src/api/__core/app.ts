...
import { createCheckout, createPortal, getBillingStatus, handleStripeWebhook } from "../routes/billing";

export interface RpcContext { headers: Headers; }
export const base = os.$context<RpcContext>();

function allowedOrigins() {
  return new Set([process.env.APP_ORIGIN, process.env.CORS_ORIGINS, "http://localhost:3000", "http://localhost:5173"]
...
export function createApp(router: Router<Record<never, never>, RpcContext>) {
  ...
  const app = new Hono().use(cors({ origin: (origin) => (origin && origins.has(origin) ? origin : undefined), credentials: true }));
  ...
  app.post("/api/billing/portal", async (c) => {
    const result = await createPortal(c.req.raw);
    return c.json(result, 200, { "cache-control": "no-store" });
  });

  app.post("/api/billing/webhook", async (c) => c.json(await handleStripeWebhook(c.req.raw), 200));

  const handler = new RPCHandler(router);
  app.use("/api/rpc/*", async (c, next) => {
    const session = getOrCreateSession(c.req.raw);
    const request = withSessionHeader(c.req.raw, session.token);
    const { matched, response } = await handler.handle(request, { prefix: "/api/rpc", context: { headers: request.headers } });
